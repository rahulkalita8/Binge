...
const extractRottenTomatoRating = (ratingsArray) => {
  if (ratingsArray.length > 0) {
    const rottenTomatoRating = ratingsArray[1] ? ratingsArray[1].Value : "N/A";
...
const fetchRatings = async (title) => {
  ...
  try {
    const omdbResponse = await fetch(encodeURI(`https://www.omdbapi.com/?apikey=${omdbApiKey}&t=${title}`));

    const omdbResponseJson = await omdbResponse.json();
    if (omdbResponseJson["Error"] && omdbResponseJson["Error"] === MOVIE_NOT_FOUND) {
      console.log("OMDB unable to find given movie");
    } else {
      defaultRatings = {
        ...defaultRatings,
        imdbRating: omdbResponseJson.imdbRating,
        rottenTomato: extractRottenTomatoRating(omdbResponseJson.Ratings || []),
